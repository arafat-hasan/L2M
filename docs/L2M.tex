...
\author{
  \IEEEauthorblockN{Md Arafat Hasan Jenin}
  \IEEEauthorblockA{
  ...
  \and
  \IEEEauthorblockN{Mohammad Motiur Rahman}
  \IEEEauthorblockA{
    \textit{Department of Computer Science and Engineering}\\
    \textit{Mawlana Bhashani Science and Technology University}\\
    Tangail, Bangladesh\\
    motiurcse@mbstu.ac.bd
  }
}
...
text---is particularly demanding, imposing four tightly coupled requirements
on any generation system.

Effective lyrics-to-melody generation demands \textbf{semantic understanding}
of lyrical content and emotional register: the system must interpret not merely
the denotative meaning of words but the affective intent and thematic
atmosphere embedded in the text. Equally essential is precise \textbf{syllabic
alignment}---every phonetic syllable must correspond to exactly one musical
note, preserving the natural cadence of the sung phrase and ensuring
singability.

Beyond these text-level constraints, the output must exhibit \textbf{musical
coherence}: a well-chosen key, consistent tempo, appropriate mode, and a
melodic contour that forms a satisfying phrase arc within the conventions of
tonal music. Finally, and most critically, the melody must achieve
\textbf{emotional consistency}---the affective character of the musical output
must reinforce, rather than contradict, the sentiment expressed in the lyrics.
...
\begin{itemize}
  \item Require large paired lyrics-melody training corpora for model training;
  \item Provide no graceful degradation when the model fails at inference time;
  \item Produce only a single output format (typically MIDI only);
  \item Apply no explicit emotion-to-key grounding in the generation process;
  \item Are not available as deployable, open-source software packages.
\end{itemize}

This paper presents \textbf{L2M}, a system that directly addresses all five
gaps. The principal contributions are:
...
\begin{enumerate}
  ...
  \item \textbf{Prompt Engineering Methodology.} Structured few-shot prompts
        with explicit output schemas, constraint enforcement (\textit{``EXACTLY
        $N$ notes for $N$ syllables''}), and contextual carryover across
        long-lyric chunks---providing a reusable template for other cross-modal
        LLM tasks.
...
\caption{L2M six-stage sequential pipeline. Blue stages invoke LLM API
calls; each has a deterministic fallback (orange). Stage~6 (dashed) is
optional and requires FluidSynth.}
\label{fig:pipeline}
\end{figure}
...
$\to$ \texttt{MIDI/MusicXML/WAV/MP3 files}.
