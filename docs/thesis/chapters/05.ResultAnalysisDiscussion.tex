...
Five findings emerge from the evaluation, one for each of the research
questions posed in \Autoref{chap:intro}.
...
\textbf{Finding 2 --- Two-stage decomposition (RQ2).} Separating emotion
...
\textbf{Finding 3 --- Value of explicit emotion grounding (RQ3).} Supplying
the emotion-to-musical-attribute mapping as prompt guidance yielded 95\%
emotion--key consistency and 90\% tempo appropriateness, with the single
inconsistent case arising from genuinely ambiguous mixed sentiment rather
than from a mis-selected key. Grounding the model in an explicit table is
therefore an effective substitute for learning affective conventions from
paired data.

\textbf{Finding 4 --- Hybrid architecture value (RQ4).} The fallback
...
\textbf{Finding 5 --- Prompt engineering as a core technical contribution
(RQ5).} The syllable-count hard constraint proved critical: without explicit
...
Four limitations bound the present work:

\begin{enumerate}
    \item \textbf{Musical scope.} Generated melodies are monophonic and
    only locally coherent. Chord progressions, harmonization, and
    countermelody are outside the current scope, and the chunking strategy
    maintains local continuity through three-note context carryover without
    enforcing global structure (e.g., a return to the tonic at phrase
    boundaries).
