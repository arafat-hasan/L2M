...
class Config:
    ...
    def _safe_int(
        ...
        """
        return int(Config._safe_float(env_var, float(default),
    ...
    def _safe_url(env_var: str, default: str) -> str:
        ...
        if not Config._validate_url(url):
            _config_logger.warning(
    ...
    TEMPERATURE: float = _safe_float.__func__("TEMPERATURE", 0.7, 0.0, 2.0)
    MAX_TOKENS: int = _safe_int.__func__("MAX_TOKENS", 1500, 1, 32000)
    API_BASE: str = _safe_url.__func__("API_BASE", "https://api.openai.com/v1")
    API_TIMEOUT: int = _safe_int.__func__("API_TIMEOUT", 30, 1, 300)
